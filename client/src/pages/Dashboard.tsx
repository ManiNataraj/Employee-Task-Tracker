...
const priorityConfig: Record<string, { badge: string; dot: string }> = {
  ...
  Low:    { badge: "bg-emerald-50 text-emerald-700 border border-emerald-200", dot: "bg-emerald-400" },
};
...
function getDueDateStatus(dateStr: string | undefined, isDone: boolean) {
  ...
  if (diffDays < 0)  return { label: "Overdue",      cls: "bg-rose-50 text-rose-700 border border-rose-200"   };
  if (diffDays === 0) return { label: "Due today",   cls: "bg-amber-50 text-amber-700 border border-amber-200" };
  if (diffDays <= 2) return { label: `${diffDays}d left`, cls: "bg-orange-50 text-orange-700 border border-orange-200" };
  return { label: formatDueDate(dateStr), cls: "bg-gray-50 text-gray-500 border border-gray-200" };
}
...
export default function Dashboard() {
  ...
  const filteredTasks = tasks.filter((task) => {
    const statusMatch =
      ...
      filter === "Pending"   ? task.status === "Pending"   : true;
  ...
  // shared input class
  const inputCls = "px-3.5 py-2.5 text-sm border border-gray-200 rounded-lg bg-white text-gray-900 placeholder-gray-400 outline-none focus:border-gray-900 focus:ring-2 focus:ring-gray-900/10 transition";
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 sm:py-10">
        ...
        <div className="mb-7">
          <div className="flex items-center gap-3 mb-1">
            ...
            <h1 className="text-xl sm:text-2xl font-bold text-gray-900 tracking-tight">Task Manager</h1>
          </div>
          <p className="text-sm text-gray-500 ml-12">Track and manage your work efficiently</p>
        </div>
          ...
          <div className="mb-6 space-y-3">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div className="bg-white rounded-xl border border-gray-200 p-3 sm:p-4">
                <p className="text-[10px] sm:text-xs font-semibold text-gray-400 uppercase tracking-widest mb-1">Total</p>
                <p className="text-2xl sm:text-3xl font-bold text-gray-900">{tasks.length}</p>
              </div>
              <div className="bg-white rounded-xl border border-gray-200 p-3 sm:p-4">
                <p className="text-[10px] sm:text-xs font-semibold text-gray-400 uppercase tracking-widest mb-1">Pending</p>
                <p className="text-2xl sm:text-3xl font-bold text-amber-500">{pendingCount}</p>
              </div>
              <div className="bg-white rounded-xl border border-gray-200 p-3 sm:p-4">
                <p className="text-[10px] sm:text-xs font-semibold text-gray-400 uppercase tracking-widest mb-1">Done</p>
                <p className="text-2xl sm:text-3xl font-bold text-emerald-500">{completedCount}</p>
              </div>
              <div className="bg-white rounded-xl border border-gray-200 p-3 sm:p-4">
                <p className="text-[10px] sm:text-xs font-semibold text-gray-400 uppercase tracking-widest mb-1">Overdue</p>
                <p className={`text-2xl sm:text-3xl font-bold ${overdueCount > 0 ? "text-rose-500" : "text-gray-300"}`}>{overdueCount}</p>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-3 sm:p-4">
              <div className="flex justify-between items-center mb-2">
                <p className="text-[10px] sm:text-xs font-semibold text-gray-400 uppercase tracking-widest">Progress</p>
              ...
              <div className="flex justify-between mt-1.5">
                <span className="text-[10px] text-gray-400">{completedCount} completed</span>
                <span className="text-[10px] text-gray-400">{tasks.length} total</span>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-3 sm:p-4 mb-4 shadow-sm space-y-3">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest hidden sm:inline">Status</span>
                ...
                {["All", "Pending", "Completed"].map((f) => (
                  <button
                    ...
                    className={`px-3 sm:px-4 py-1.5 text-xs sm:text-sm font-medium rounded-full transition-all ${
                      filter === f ? "bg-gray-900 text-white shadow-sm" : "text-gray-500 hover:text-gray-700 hover:bg-gray-200"
                    }`}
                  >
                    {f}
                    {f === "All" && tasks.length > 0 && (
                      <span className={`ml-1.5 text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${filter === "All" ? "bg-white/20 text-white" : "bg-gray-200 text-gray-500"}`}>
                        {tasks.length}
                      </span>
                    )}
                  </button>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <div className="flex gap-1.5 flex-wrap">
              ...
              >
                ...
                High
                <span className={`text-[10px] font-semibold px-1 py-0.5 rounded-full ${priorityFilter === "High" ? "bg-white/20 text-white" : "bg-rose-100 text-rose-600"}`}>
              ...
              >
                <span className={`w-1.5 h-1.5 rounded-full ${priorityFilter === "Medium" ? "bg-white" : "bg-amber-400"}`} />
                Medium
                <span className={`text-[10px] font-semibold px-1 py-0.5 rounded-full ${priorityFilter === "Medium" ? "bg-white/20 text-white" : "bg-amber-100 text-amber-600"}`}>
              ...
              >
                <span className={`w-1.5 h-1.5 rounded-full ${priorityFilter === "Low" ? "bg-white" : "bg-emerald-400"}`} />
                Low
                <span className={`text-[10px] font-semibold px-1 py-0.5 rounded-full ${priorityFilter === "Low" ? "bg-white/20 text-white" : "bg-emerald-100 text-emerald-600"}`}>
        ...
        {filteredTasks.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-gray-400">
            <svg width="40" height="40" fill="none" viewBox="0 0 24 24" className="mb-3 opacity-30">
              ...
              return (
                <div
                  ...
                  className={`bg-white rounded-xl border p-4 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 ${
                    isOverdue ? "border-rose-200 bg-rose-50/30" : isDone ? "border-gray-100 bg-gray-50/50" : "border-gray-200"
                  }`}
                >
                  {/* Top: checkbox + title */}
                  <div className="flex items-start gap-2.5 mb-2.5">
                    <div className={`mt-0.5 w-5 h-5 rounded-md flex-shrink-0 flex items-center justify-center transition-colors ${isDone ? "bg-emerald-500" : "border-2 border-gray-300 bg-white"}`}>
                      {isDone && (
                    ...
                    {editingId === task.id ? (
                      ...
                      <h2 className={`flex-1 text-sm font-semibold leading-snug break-words ${isDone ? "line-through text-gray-400" : "text-gray-900"}`}>
                        {task.taskName}
                      </h2>
                    )}
                  </div>
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2 ml-7">
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap">
                      {/* Priority */}
                      <span className={`inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full ${pc.badge}`}>
                        <span className={`w-1.5 h-1.5 rounded-full ${pc.dot}`} />
                        {task.priority}
                      </span>
                      {/* Status */}
                      <span className={`text-xs font-medium px-2.5 py-1 rounded-full border ${isDone ? "bg-emerald-50 text-emerald-700 border-emerald-200" : "bg-orange-50 text-orange-700 border-orange-200"}`}>
                        {task.status}
                      </span>
                      {/* Due date badge */}
                      {dueMeta && (
                        <span className={`inline-flex items-center gap-1 text-xs font-medium px-2.5 py-1 rounded-full ${dueMeta.cls}`}>
